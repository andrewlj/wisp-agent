...
- **Long-term memory** — tell wisp a fact/preference (`remember`); it persists in `memory.md` across sessions/restarts and is recalled automatically
...
└── memory.md           # long-term user facts/preferences (via remember)
```
...
├── memory.py             # Long-term personal memory (read/write memory.md)
